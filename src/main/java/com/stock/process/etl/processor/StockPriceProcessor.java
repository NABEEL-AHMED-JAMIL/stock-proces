package com.stock.process.etl.processor;

import com.google.gson.Gson;
import com.google.gson.JsonArray;
import com.google.gson.JsonObject;
import com.stock.process.ai.StockDataAIService;
...
public class StockPriceProcessor implements Runnable {
    ...
    private StockDataAIService stockDataAIService;
    private EfsFileExchange efsFileExchange;
    private FileInfoRepository fileInfoRepository;
    private AuditLogRepository auditLogRepository;

    public StockPriceProcessor() {}

    public StockPriceProcessor setStockDataAIService(StockDataAIService stockDataAIService) {
        this.stockDataAIService = stockDataAIService;
        return this;
    }

    public StockPriceProcessor setEfsFileExchange(EfsFileExchange efsFileExchange) {
        this.efsFileExchange = efsFileExchange;
        return this;
    }

    public StockPriceProcessor setFileInfoRepository(FileInfoRepository fileInfoRepository) {
        this.fileInfoRepository = fileInfoRepository;
        return this;
    }

    public StockPriceProcessor setAuditLogRepository(AuditLogRepository auditLogRepository) {
        this.auditLogRepository = auditLogRepository;
        return this;
    }

    public StockPriceProcessor setFileInfo(FileInfo fileInfo) {
    ...
    public void run() {
        ...
        try {
            ...
            String fileType = this.fileInfo.getType().toUpperCase();
            switch (fileType) {
                case CSV:
                case TXT:
                    readCSVFile();
    ...
    private void readCSVFile() throws Exception {
        ...
        String extension = this.fileInfo.getType().equalsIgnoreCase(CSV) ? CSV_EXT : TXT_EXT;
        String segmentPath = this.fileInfo.getFolder() + "/" + this.fileInfo.getFilename().replace(extension, JSON_EXT);
        this.fileInfo.setSegmentPath(segmentPath);
        Gson gson = new Gson();
        File file = this.efsFileExchange.getFile(this.fileInfo.getPath());
        JsonObject jsonObject = this.getJsonObject(FileUtils.readLines(file, StandardCharsets.UTF_8));
    ...
    private JsonObject getJsonObject(List<String> lines) {
        ...
        for (String line : lines) {
            ...
            if (isHeader) {
                ...
                rows.add(dataParse);
                //this.stockDataAIService.embedStockData(line, this.fileInfo);
            }
        }
    ...
    /**
     * Method use to create text row
     * @param values
     * @param statisticsArray
     * @return JsonArray
     * */
    private JsonArray createTextRow(String[] values, DescriptiveStatistics[] statisticsArray) {
